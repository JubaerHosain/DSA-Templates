...
void solve() {
    ...
    for(int i = 0; (ll)prime[i]*(ll)prime[i] <= r; i++) {
        int curr = (l/prime[i])*prime[i];       //first occurance of multiple of prime[i]
        if(curr < l) curr += prime[i];          //in the range l to r

        for(int j = curr; j <= r; j+=prime[i]) {
            if(j != prime[i]) pm[j-l] = false;
        }
    }
